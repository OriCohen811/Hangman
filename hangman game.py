...
def main():
    open_screen()
    SECRET_WORD = choose_word()  # the secret word
    if SECRET_WORD == None :
        print("Problem occurred\n")
        return
    old_letters_guessed = []  # guesses list
    mistakes = count_mistakes(old_letters_guessed, SECRET_WORD)
    print(mistakes_draws())
    print(show_hidden_word(SECRET_WORD, old_letters_guessed))
    while mistakes < 6:
        # check if the letter is valid, if is valid return the new letter and add to list
        guess_letter(old_letters_guessed)
        mistakes = count_mistakes(old_letters_guessed, SECRET_WORD)
        draw = mistakes_draws(mistakes)  # return draws of the hungman
        secret_word_situation = show_hidden_word(SECRET_WORD, old_letters_guessed)  # return secret word situation
        if SECRET_WORD == secret_word_situation.replace(" ", ''):
            # when all the secret word letters guessed
            print(SECRET_WORD)
            print('WINNER!!!!!')
            return
        draw_and_hidden_word(draw, secret_word_situation)  # print draw and secret word
        print(mistakes)
    if mistakes == 6 :
        print(SECRET_WORD)
        print('\n\nOhh... Such a bad luck :( \nSee you next time!!\n')
...
if __name__ == "__main__":
    while True:  # for the game start all over again
        main()
